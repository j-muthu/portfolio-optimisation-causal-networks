...
object: the covariance matrix of returns or, equivalently, the correlation
matrix. Its most successful recent representative, Hierarchical Risk Parity
(HRP) \cite{lopezdeprado2016hrp}, avoids the well-documented instability of
inverting a noisy covariance matrix by clustering assets on a
correlation-derived distance and allocating by recursive bisection down the
resulting tree. Clustering is the first step of the algorithm, and its input
...
The answer matters to three audiences. For the practitioner, Q is a
question about costs: symmetrising a graph is free, exploiting orientation
...
The thesis addresses a single question, used throughout the report:
...
\begin{itemize}[itemsep=1pt, topsep=3pt]
  ...
  \item \emph{A discovered causal graph}: the contemporaneous asset--asset
    ...
    the decomposition is measured as a function of estimation horizon
    rather than at a single conventional window.
...
\begin{enumerate}[label=\textbf{R\arabic*}, itemsep=1pt, topsep=3pt]
  \item \emph{The causal graph must improve on the correlation matrix at
    all}, since otherwise there is no gain to decompose. Tested
    like-for-like: the
...
rung}, $\skelsamp{}-\HRP{}$, measures the value of knowing which pairs are
linked; the \emph{orientation rung}, $\orientany{}-\skelsamp{}$ for each
orientation-aware variant $\orientany{}$, measures the additional value,
holding the graph fixed, of knowing the direction of each link. The two
rungs sum to the total causal-graph gain, so nothing is left unattributed.
Success is therefore any precisely estimated, rigorously adjudicated answer
at the two rungs, including a null: the failure mode the design guards
...
\begin{enumerate}[label=\textbf{A\arabic*}, itemsep=2pt, topsep=3pt]
  \item \emph{A reproducible rolling pipeline for causal discovery}
    ...
    (${\sim}700\times$ fan-out speedup); a data-determinism defect found
...
\begin{itemize}[itemsep=2pt, topsep=3pt]
  \item \textbf{The decomposition (the answer to Q).} The graph's total gain
    ...
    orientation rung is positive at every window and U-shaped
    ...
    ($p=\rsSpaSkelWthree$, $\rsSpaSkelWfive$), and the orientation
    family's edge over the skeleton clears it at two years
    ($p=\rsSpaDirWfive$). These rejections are marginal
    ($p \geq \rsSpaSkelWthree$ across the eight skeleton/orientation
    $\times$ window tests) and coincide with the windows where the point
    estimates peak.
  \item \textbf{What orientation buys is robustness, not level.} The
    ...
    four horizons, against the skeleton-only allocator's
    ...
    two-year windows), consistent with causal structure substituting for
    sample-moment quality. In point estimates the edge is invariant to
    transaction costs up to 20\,bps and to the graph's sparsity threshold,
    and \orientsem{} and \skelsem{} at the 18-month window attain the two
    highest Deflated Sharpe ratios of all \rsNtrials\ configurations
    ($\rsDtwosDSRWthree$, $\rsDoneDSRWthree$). The effect is, however,
    method-dependent (absent under VARLiNGAM graphs at all four windows)
    and, descriptively, concentrates in calm (bottom-VIX-quintile) regimes
    rather than stress.
...
stable across estimation horizons, but its source shifts: the skeleton pays
only near the conventional one-year window, while the orientations pay at
every horizon and most where sample moments are weakest. Family-wise
support reaches conventional significance only at the longest horizons,
marginally, where the point estimates also peak.} Establishing that
decomposition, with its uncertainty stated, is the contribution.

\section{Report structure}
\label{sec:structure}

Chapter~\ref{chap:background} analyses the three works this study builds
on (HRP and the backtest-rigour agenda, causal graphs in equity markets,
and HSP) and locates the gap. Chapter~\ref{chap:discovery} (A1) presents the
