...
bisection (Section~\ref{sec:d2}). Second, \emph{every input to steps
...
hierarchical allocator can represent that fact. This is the \emph{interface} deficit of
...
methods. Q sharpens that claim into a decomposition, asking whether the
...
\cite{rodriguez2023hsp,rodriguez2025causal}: if the drivers are in fact the
...
\emph{The selector}: the drivers are chosen by cumulative correlation
with the assets, the very quantity the causal motivation was meant to
replace, and the selection is demonstrably tautological in practice: on
...
encounters a measurement problem, in that at a monthly rebalance the
feedback reward
...
The evaluation shows that whole route to be dominated by allocating from
the asset--asset graph directly; this result reoriented the study to Q, and
it is why the driver route appears in this report as a documented negative
result rather than the destination.
...
graph; none allocates from an asymmetric matrix at all; and consequently
none can decompose the graph's allocation value into skeleton and
orientation, which is the decomposition Q demands and
