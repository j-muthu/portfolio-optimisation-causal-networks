...
project's starting framework; and Section~\ref{sec:cache} the engineering
...
are the selection pool for the driver route
(Section~\ref{sec:driver-route}) and, more importantly for Q, their
presence in the joint fit means the
...
\emph{natively}, rather than by post-hoc edge deletion; and it must return
...
the graph that Q's allocators consume.
...
Because Q hinges on what the orientations contribute, the design
...
\textbf{VARLiNGAM} \cite{hyvarinen2010varlingam} supplies
\emph{differently-identified} orientations: its instantaneous matrix $B_0$
...
Chapter~\ref{chap:evaluation} observes. \textbf{GRANGER} supplies
\emph{magnitude-degraded} orientations: a ridge-regularised VAR(1) on the
...
should not reproduce it; if it needs trustworthy edge \emph{magnitudes}
rather than arrows alone, GRANGER's ordering should survive where its
shock propagation fails, which is the dissociation
Section~\ref{sec:decomposition} observes. (A
...
block to exactly zero and its discovered structure agrees with DYNOTEARS
only modestly, indicating a distinct structural lens, but at
${\sim}10\times$ the fit
...
shows that zero drivers clear 5\% significance
...
points at essentially every rebalance. Q, however, does not rest on this
arm.
...
reuse one fit. The effect is decisive for Q: the full ablation grid (every
allocator--method--window cell, plus the sparsity, cost and comparator
...
instead of ${\approx}15$ hours; populating a new estimation window costs
one overnight run rather than weeks. The cache also
guarantees the ablation's
premise: allocators cannot see different graphs, because there is exactly
one graph per (method, window, data) key.
...
changes of ${\approx}0.14$ in $\lVert W\rVert$. The pipeline was therefore
not reproducible at the bit level, and two previously-drawn conclusions (an
...
robustness result in its own right. All numbers in this report come from
the corrected, deterministic pipeline: two independent runs now produce
identical windows, weights and NAVs, verified end-to-end by the
