...
\paragraph{D0s --- a second symmetrisation.} Hierarchical clustering demands
...
recipe rather than about the skeleton. D0s is the control on that control: the
...
\begin{table}[h]
  ...
  \caption{The two doors, crossed. The row is where the ordering comes from,
    the column is what recursive bisection splits on; D0 is the control with
    both doors shut to orientation.}
  \label{tab:crossing}
  \begin{tabular}{l l l}
    \toprule
    Ordering from & splits on sample cov. & splits on $\Sigma_{\mathrm{struct}}$ \\
    \midrule
    clustering on the skeleton & D0 (control) & D1 \\
    topological sort           & D2           & D2s \\
    \bottomrule
  \end{tabular}
\end{table}
...
control in exactly one respect --- D1 in the covariance alone, D2 in the
ordering alone, D2s in both --- so the results chapter can localise not just
