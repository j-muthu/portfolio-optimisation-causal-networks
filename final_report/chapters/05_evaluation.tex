...
\section{R1: the causal graph against the correlation matrix}
\label{sec:r1}
...
noise would produce, though no cell is individually significant. The
family-wise test (Hansen's SPA, the reported causal-structure family
against
...
marginally, at a size Section~\ref{sec:believe-rigour} weighs.
...
plain correlation-distance HRP itself beats V0 ($\rsCorrSharpe$ vs $0.371$
at
...
statement about V0's machinery rather than about the correlation matrix.
Stating Q
...
(V2) is exactly inert: across the full
...
skeleton rung is hump-shaped ($-0.003$ at nine months, peaking at
$+0.022$ at the one-year convention, decaying to $+0.009$ at two years) and
never individually significant ($p \geq 0.14$). Third, the
orientation rung is positive at every window and U-shaped ($+0.021$ at
$p=0.08$, $+0.009$ at $p=0.47$, $+0.012$ at $p=0.33$, $+0.023$ at
$p=0.049$): largest at the two extremes, smallest where the skeleton peaks,
...
source of extra return; the initial two-window design could only suggest
this mechanism (skeleton-carried at one year, orientation-carried at
two), and the four-point gradient now supports it at every horizon
measured.
...
structured estimator (fewer effective parameters, shrinkage-like
in character), so a sceptic could read the two-year gain as a generic
structured-covariance effect rather than a directional one. Two
observations argue against this. First, orientation is constitutive of the
estimator, not a decoration on it: the exact total-effect algebra of
Section~\ref{sec:sem} exists only because the directed graph is acyclic,
so
...
worked. The gain therefore tracks the content of the DYNOTEARS
...
skeleton control drifts down ($0.403\to0.394$), so the orientation-aware
...
costs rise from 0 to 20\,bps, because \skelsem{}'s turnover ($0.102$
one-way per
rebalance) is below the control's ($0.112$). Orientation does not win
...
The magnitude-degraded GRANGER control then localises where inside
...
that the orientation rung's value does not lie in merely knowing which way
the
...
\skelsamp{} by $+0.025$
($p=0.008$), the only $p<0.01$ contrast in the study, and a
control-versus-control comparison. The implication is that at the shortest
window
...
regime-conditional analysis contradicts this (Figure~\ref{fig:regime}): at
the
...
expected from \rsNtrials\ no-skill trials, the two highest Deflated
Sharpe ratios in the entire study belong to the orientation-aware
allocators at the 18-month window, \orientsem{} ($\rsDtwosDSRWthree$) and
\skelsem{}
...
where the pairwise orientation effect also peaks. \textbf{(iv)} The
Chapter-\ref{chap:sensitivity} allocator family against V0: $p=\rsSpaDvar$,
not significant even against the weak anchor. In sum, four of the
...
longest horizons of families (ii) and (iii), coinciding with the windows
where the pairwise
...
suggestive but short of decisive, that the causal-structure edge at the
...
consistently, the HSP baseline V0. The MCS confirms V0's
...
deterministic graph-based family, every member of which sits above the seed
...
moderate and reported at its estimated size: 36 of 40
graph-versus-correlation cells
positive; one pairwise-significant orientation cell among twelve
($p=0.049$) and one suggestive one ($p=0.08$); and family-wise rejections
...
$p=\rsSpaDirWfive$), marginal, and coinciding with the peaks of the point
estimates. The largest defensible claim is conditional: if one
...
landing where that pattern says they should.
...
everywhere, largest at the extremes) would have been weakened,
and reported as such, had the new windows come back inconsistent. The
...
regulariser role of orientation; a family-wise edge over the stronger
controls
at the longest horizons, marginal and reported as such; and the dominance
of
...
family-wise support concentrated where those estimates say it
