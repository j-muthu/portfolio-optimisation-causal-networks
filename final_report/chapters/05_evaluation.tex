...
(215 rebalances at the one-year window; the two-year window's longer burn-in
leaves 203), estimation windows of 252 and 504 trading days for both the
...
$+0.022$ ($p=0.14$); adding orientation on top lifts the best treatment (D1)
a further $+0.009$, a total causal-graph gain of $+0.031$ ($p=0.14$;
Table~\ref{tab:decomp}). At the two-year window
$\textsc{corr}=\rsCorrSharpeWfive$ and the total gain is $+0.032$--$0.036$
(D1/D2s, $p=0.12$/$0.084$). All 28 graph-versus-\textsc{corr} contrasts ---
seven graph-consuming allocators, both discovery methods, both windows ---
are positive, consistent in direction across the entire grid, but \emph{none
is individually significant}, and the family-wise test (Hansen's SPA, all
causal-structure strategies against \textsc{corr}) gives $p=\rsSpaSkelWtwo$
at one year and $p=\rsSpaSkelWfive$ at two. R1 is therefore supported
directionally but not statistically: the causal graph consistently
out-performs the correlation matrix by a modest margin that data-snooping
correction cannot distinguish from a lucky search.
...
Figure~\ref{fig:decomp} is the thesis's central result;
...
not survive: Hansen's SPA over the direction-aware variants against their
...
\begin{figure}[tbp]
  ...
  \caption{Net Sharpe by discovery method $\times$ allocator at both windows
    (boxed column: the symmetrised control D0). The one-year panel is
    skeleton-strong; in the two-year panel the DYNOTEARS row is
    orientation-strong, while the VARLiNGAM rows show no orientation effect
    at either window.}
  \label{fig:heatmap}
\end{figure}
...
\begin{figure}[tbp]
  ...
  \caption{The fixed-graph orientation effect: $\Delta$Sharpe of each
    ...
    contrasts are positive at the two-year window (D1 significant pairwise);
    at one year, and under VARLiNGAM at both windows, the contrasts scatter
    around zero and none is significant.}
  \label{fig:forest}
\end{figure}
...
\textbf{(iv)} The Chapter-\ref{chap:sensitivity} allocator family against
V0: $p=\rsSpaDvar$ --- not significant even against the weak anchor. The
pattern: the study's sole family-wise rejection, (i), anchors on the weak
incumbent baseline and is carried by the skeleton-bearing variants; every
test against an honest control fails to reject. Family-wise, the only
hypothesis this study can reject is that the incumbent's machinery is
competitive.
...
\emph{statistical support} is honest but thin: consistent signs across all
28 graph-versus-correlation contrasts, one pairwise-significant orientation
cell ($p=0.049$), and no family-wise survival anywhere. The largest
defensible claim is conditional: \emph{if} one replaces the correlation
matrix with a DYNOTEARS graph, the skeleton is what pays at short windows,
and keeping the orientations costs nothing while materially improving
robustness to the estimation-window choice.
