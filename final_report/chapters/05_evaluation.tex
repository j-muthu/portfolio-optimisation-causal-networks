...
comparison is the graph-blind \textsc{corr} control against the causal-graph
...
graph treatment (D1) delivers a total causal-graph gain over it of $+0.018$,
$+0.031$, $+0.030$ and $+0.032$ respectively ($p=0.43$--$0.12$;
Table~\ref{tab:decomp}), with D2s reaching $+0.036$ at two years. Of the 56
graph-versus-\textsc{corr} cells --- seven graph-consuming allocators, both
discovery methods, four windows --- 52 are above the control (27 of 28 under
...
\textsc{corr}) gives $p=\rsSpaSkelWone$, $\rsSpaSkelWtwo$,
...
\emph{total} causal-graph gain is stable: $+0.018$ to $+0.032$ (D1) at every
horizon, rising to $+0.036$ for D2s at two years. Second, the
...
with 16 of the 20 DYNOTEARS orientation contrasts positive across the grid
...
\begin{figure}[tbp]
  ...
  \caption{The central result: the two rungs of the decomposition as a
    ...
    skeleton addition (D0 $-$ \textsc{corr}, hump-shaped) and the
    orientation addition (D1 $-$ D0, U-shaped, positive throughout) with
...
\begin{table}[t]
  ...
  \begin{tabular}{l c c c c}
    ...
    \midrule
    skeleton\ (D0 $-$ \textsc{corr}) & $-0.003$ (0.82) & $+0.022$ (0.14) & $+0.018$ (0.23) & $+0.009$ (0.55) \\
    orientation, D1\ (D1 $-$ D0)     & $+0.021$ (0.08) & $+0.009$ (0.47) & $+0.012$ (0.33) & $+0.023$ (0.049) \\
    orientation, D2s\ (D2s $-$ D0)   & $+0.021$ (0.21) & $+0.003$ (0.80) & $+0.017$ (0.27) & $+0.027$ (0.067) \\
    \midrule
    total, D1\ (D1 $-$ \textsc{corr})   & $+0.018$ (0.43) & $+0.031$ (0.14) & $+0.030$ (0.16) & $+0.032$ (0.12) \\
    total, D2s\ (D2s $-$ \textsc{corr}) & $+0.019$ (0.41) & $+0.026$ (0.24) & $+0.035$ (0.099) & $+0.036$ (0.084) \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[tbp]
  ...
  \caption{The per-window levels behind Figure~\ref{fig:gradient}
    (DYNOTEARS graphs, net annualised Sharpe): \textsc{corr} $\to$ D0
    ($+$skeleton) $\to$ D1/D2s ($+$orientation) at each estimation window,
...
\begin{figure}[tbp]
  ...
  \caption{Net Sharpe by discovery method $\times$ allocator at the four
    windows (boxed column: the symmetrised control D0; panel titles carry
    the \textsc{corr} baseline). The DYNOTEARS orientation-aware cells (D1,
    D2s) are strong at every window --- D2s at 378 days (0.419) is the
...
\begin{figure}[tbp]
  ...
  \caption{The fixed-graph orientation effect: $\Delta$Sharpe of each
    orientation-aware allocator against the skeleton control D0 at all four
    windows, with $95\%$ block-bootstrap CIs ($\ast$: $p<0.05$). Sixteen of
    twenty DYNOTEARS contrasts are positive (D1 starred at two years); the
    VARLiNGAM contrasts scatter around zero (6/20 positive). The starred
    D0s cell at the nine-month window is a control-versus-control
...
windows, and below the control at nine months); D1 ($0.395$--$0.415$) and
D2s ($0.399$--$0.419$) are strong everywhere (Figure~\ref{fig:nav} shows the
...
contrast $\text{D1}-\text{D0}$ compares the SEM-implied covariance against
...
is absent at all four windows (6 of 20 contrasts positive, mean $-0.004$;
...
Under the cheap GRANGER graphs the split is informative: causal-ordered
bisection survives (D2 $=0.393$ vs DYNOTEARS' $0.399$) but
the structural-covariance allocators collapse (D1 $=0.352$, D3 $=0.350$)
--- a ridge-VAR coefficient matrix supports an \emph{ordering} but its
magnitudes cannot support shock propagation. Expensive discovery earns its
cost only for the allocators that consume edge weights, and the
best-allocator gap (GRANGER $-$ DYNOTEARS at D1: $-0.060$, $p=0.12$)
favours DYNOTEARS without reaching significance.

\paragraph{The controls disagree at the shortest window.} At 189 days the
two orientation-blind symmetrisations diverge: D0s beats D0 by $+0.025$
...
allocators at the 18-month window --- D2s ($\rsDtwosDSRWthree$) and D1
...
family vs the \textsc{corr} control, per window: $p=\rsSpaSkelWone$,
...
$\rsSpaDirWthree$, $\rsSpaDirWfive$ --- closest to rejection at the two
...
deterministic D-family, every member of which except D4 sits \emph{above
the seed cloud's maximum} --- the headline allocators D0, D1 and D2s
comfortably so --- and all of which produce byte-identical results on every
run.
...
but thin: 52 of 56 graph-versus-correlation cells positive, one
pairwise-significant orientation cell among twenty ($p=0.049$), one
...
$p=0.049$ cell among twenty orientation contrasts is exactly what
