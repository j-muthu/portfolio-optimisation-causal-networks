...
source of extra return, a mechanism the four-point gradient supports at
every horizon measured.
...
\paragraph{Forking-paths risk.} Two design choices were informed by
...
windows come back inconsistent. The family-wise rejections inherit the
risk asymmetrically: the strongest (R1 at 378 days, $p=\rsSpaSkelWthree$)
sits on an added window, while the two-year rejections
($p=\rsSpaSkelWfive$, $\rsSpaDirWfive$) sit on windows evaluated from the
start, which bounds the concern for them. Finally, the family-wise tests
adjudicate the allocator family the report defines; the deflation universe
prices every configuration the search ever evaluated, including
exploratory arms outside that family.
