...
noise would produce, though \emph{no cell is individually significant}. The
family-wise test (Hansen's SPA, the reported causal-structure family against
\HRP{}) gives $p=\rsSpaSkelWone$, $\rsSpaSkelWtwo$,
$\rsSpaSkelWthree$ and $\rsSpaSkelWfive$ across the four windows ---
strengthening with the horizon and crossing the $5\%$ bar at the two
longest. R1 is therefore supported directionally at every window and
family-wise at the longer horizons: the causal graph consistently
out-performs the correlation matrix by a modest margin, and at 18 months
and two years that margin survives having the family search priced in ---
marginally, a size Section~\ref{sec:believe-rigour} weighs.
...
Hansen's SPA over the direction-aware variants against their own skeleton
control gives $p=\rsSpaDirWone$, $\rsSpaDirWtwo$, $\rsSpaDirWthree$ and
$\rsSpaDirWfive$ across the four windows --- rejecting at the $5\%$ level
only at two years, exactly where the pairwise effect peaks;
...
the sample covariance, and $\Sigma_{\mathrm{SEM}}$ is also a
...
there is no orientation-free version of $\Sigma_{\mathrm{SEM}}$ to
...
control would require a symmetric analogue of $\Sigma_{\mathrm{SEM}}$,
...
graphs (\orientsamp{} $=0.393$ vs DYNOTEARS' $0.399$) while the SEM-covariance
...
test runs in all, each over the family of strategies the report defines ---
adjudicate the claims with the search priced in. \textbf{(i)}
Causal-structure strategies vs the HSP baseline V0 at one
...
with the horizon and rejecting at the two longest windows; R1 is
established family-wise there, marginally, and nowhere else.
\textbf{(iii)} Orientation-aware variants vs their own
skeleton control, per window: $p=\rsSpaDirWone$, $\rsSpaDirWtwo$,
$\rsSpaDirWthree$, $\rsSpaDirWfive$ --- rejecting only at two years,
exactly where the pairwise orientation effect peaks. \textbf{(iv)} The
Chapter-\ref{chap:sensitivity} allocator family against V0: $p=\rsSpaDvar$
--- not significant even against the weak anchor. The pattern: four of the
ten runs reject --- family (i) decisively on the weak incumbent anchor, and
three marginal rejections ($p=\rsSpaSkelWthree$--$\rsSpaDirWfive$) at the
longest horizons of families (ii) and (iii), exactly where the pairwise
point estimates peak. Two considerations size the latter: the per-window
SPA prices the search across strategies but not across the four estimation
windows, and the rejections sit just under the bar rather than deep beneath
it. They are therefore read as moderate evidence --- stronger than
suggestive, short of decisive --- that the causal-structure edge at the
longest horizons is real.
...
deterministic graph-based family, every member of which sits \emph{above the seed
...
topping the study-wide DSR table. Its \emph{statistical support} is
moderate and honestly sized: 36 of 40 graph-versus-correlation cells
positive; one pairwise-significant orientation cell among twelve
($p=0.049$), one suggestive one ($p=0.08$); and family-wise rejections
confined to the longest horizons --- R1 at 18 months and two years
($p=\rsSpaSkelWthree$, $\rsSpaSkelWfive$), orientation at two years
($p=\rsSpaDirWfive$) --- marginal, and exactly where the point estimates
peak. The largest defensible claim: \emph{if} one
...
costs nothing, materially improves robustness to the estimation-horizon
choice, and at the longest horizons delivers an edge that survives
family-wise correction at its stated, marginal size.
...
such effects are at the edge of detectability, and that cuts both ways: the
family-wise rejections that do occur are marginal
...
claimed as a discovery. The U-shape earns its standing not from any single
cell but from the sign-consistency of the orientation rung across all four
windows and both anchor allocators --- with the family-wise rejections
landing exactly where that pattern says they should.

\paragraph{Forking-paths risk.} Two of the design's axes were chosen with
results in view, and both are disclosed. The choice of \emph{question} was
informed by the first phase's results. The window grid grew from two
...
and reported as such, had the new windows come back jumbled. The
family-wise rejections inherit part of this risk asymmetrically: the
strongest (R1 at 378 days, $p=\rsSpaSkelWthree$) sits on an added window,
while the two-year rejections ($p=\rsSpaSkelWfive$, $\rsSpaDirWfive$) sit
on an original-design window, which bounds the concern for them. Finally,
the family-wise tests adjudicate the allocator family the report defines;
the deflation universe prices every configuration the search ever
evaluated, including exploratory arms outside that family.
...
regulariser role of orientation; a family-wise edge over honest controls
at the longest horizons, marginal and read as such; and the dominance of
deterministic graph-based allocation over the seed-unstable,
feedback-inert driver route.
It does not support: family-wise significance near the one-year
convention; any orientation effect under VARLiNGAM; or the stress
hypothesis. That profile --- precise point estimates, with marginal
family-wise support concentrated exactly where the point estimates say it
