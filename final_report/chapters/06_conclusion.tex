...
--- and no step of the decomposition is family-wise significant against
honest controls.} In detail (net annualised Sharpe, 2007--2024, monthly
...
\begin{enumerate}[itemsep=2pt]
  ...
  \item \textbf{Nothing survives family-wise correction against honest
        anchors.} Of the ten SPA runs across four families, Hansen's
        procedure rejects in exactly one --- the causal family against the
        weak HSP baseline ($p=\rsSpaConsistent$); against the \HRP{}
        control ($p=\rsSpaSkelWone$--$\rsSpaSkelWfive$ across windows) and
        against the skeleton control
        ($p=\rsSpaDirWfive$--$\rsSpaDirWtwo$) the decomposition's steps are
        directionally consistent but statistically unconfirmed. The $90\%$
  ...
  \item \textbf{The effect is method- and regime-specific.} Under VARLiNGAM
        ...
        \emph{ordering} survives but the structural covariance collapses ---
...
\begin{itemize}[itemsep=2pt]
  ...
  \item \textbf{Empirical:} the decomposition itself, traced across four
        ...
        horizon and largest where sample moments are weakest; nothing
  ...
  \item \textbf{Meta-scientific:} a quantified seed audit substantiating the
        ...
        whose fix overturned two previously-drawn conclusions --- with the
        allocator grid fixed before any treatment ran, every later
        extension disclosed as such, all cells cache-verified, and the
...
of Q --- though not the experiment --- was informed by the first phase's
...
$\Sigma_{\mathrm{struct}}$; but exact per-shock parity generally requires
