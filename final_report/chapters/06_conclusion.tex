...
The answer, in two lines: \emph{the causal graph's gain over the
...
significance only at the longest horizons, marginally, exactly where the
point estimates peak.} In detail (net annualised Sharpe, 2007--2024, monthly
...
\begin{enumerate}[itemsep=2pt]
  ...
  \item \textbf{Its composition shifts with the window.} The skeleton's
        ...
        (pairwise $p=0.08$/$0.47$/$0.33$/$0.049$), largest exactly where
        the sample moments are weakest. Orientation buys \emph{robustness
        to the estimation horizon}, not level: \skelsem{} ($0.395$--$0.415$) and
        \orientsem{} ($0.399$--$0.419$) are strong at every window while the
        skeleton-only allocator spans $0.372$--$0.403$, the increment is
        invariant to costs up to 20\,bps and to the graph's sparsity
        threshold, and \orientsem{} and \skelsem{} at the 18-month window attain the two
  ...
  \item \textbf{Family-wise support is marginal and confined to the longest
        ...
        exactly where the point estimates peak. Nothing rejects near the
  ...
  \item \textbf{The effect is method- and regime-specific.} Under VARLiNGAM
        ...
        \emph{ordering} survives but the SEM-implied covariance collapses ---
        so the orientation's value lies in trustworthy edge magnitudes, not
        in the arrows alone. The orientation edge concentrates in calm
  ...
  \item \textbf{The incumbent driver route is dominated.} HSP-style driver
        ...
        are deterministic, and the headline three (\skelsamp{}, \skelsem{}, \orientsem{}) exceed even
        its luckiest seed.
\end{enumerate}
...
\begin{itemize}[itemsep=2pt]
  \item \textbf{Methodological:} the orientation-aware allocator family ---
        ...
        the clustering dendrogram with the DAG's topological order --- together
  ...
  \item \textbf{Meta-scientific:} a quantified seed audit substantiating the
        ...
        harness gated on a byte-perfect replication of the prior phase's
        result.
\end{itemize}
...
\emph{structural shock origins} --- where risk enters the causal system ---
