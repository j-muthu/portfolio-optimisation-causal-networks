...
graph at every one of the 215 monthly rebalances. Section~\ref{sec:data}
describes the data; Section~\ref{sec:discovery-run} the rolling discovery, the
directional prior, and its empirical verification;
...
membership list was not available, so the S\&P~100 is approximated by taking
the top constituents by market capitalisation from historical S\&P~500
membership, yielding a fixed 99-asset universe (the union across snapshots)
over 2007--2024. Prices and point-in-time shares outstanding come from
WRDS/CRSP, with a small Yahoo Finance fallback for the most recent days.
Point-in-time CRSP data removes survivorship bias from the price series; the
top-by-market-cap construction remains an approximation of the official index
and is noted as a limitation in Section~\ref{sec:limitations}.
...
masquerade as asset-to-asset edges. Equity baskets such as US sector ETFs are
excluded from the pool as not credibly exogenous to a US large-cap portfolio.

\paragraph{Stationarity.} SVAR-based discovery assumes weakly stationary
inputs, so each series is transformed by type within each window (log-returns
for prices, first differences for yields, year-on-year changes for level
macro series), checked with both ADF and KPSS --- complementary because their
nulls are opposite, so agreement is a stronger signal than either alone.
Flags are logged rather than used to drop series, the per-type transforms
being fixed \emph{a priori} on economic grounds.
...
(252 or 504 trading days), with lag $p=1$, yielding the contemporaneous matrix
...
edge mass on economically implausible asset$\to$driver edges, and admitting
them distorts the legitimate driver$\to$asset block by 52--72\% in $L_1$
...
sparsity-robustness analysis); verifies acyclicity (a sub-block of a DAG is a
DAG); and computes the per-asset structural residual variances on the fit
window --- needed by the structural covariance of
...
\paragraph{What the graphs look like.} The DAGs are far from degenerate. At
the one-year window the DYNOTEARS asset graphs average ${\sim}625$ directed
edges (density $6.4\%$ of possible ordered pairs) with a longest directed path
of ${\approx}50$ nodes out of 99 --- deep chains of influence, not a shallow
hub-and-spoke. There is, in other words, substantial orientation structure
for the treatment allocators to exploit; whatever the evaluation finds, it
will not be because the graphs were trivially symmetric.
...
is one financial returns satisfy at short windows but progressively lose, by
aggregation, at longer ones. \textbf{GRANGER} is the deliberately cheap
comparator: a ridge-regularised VAR(1) on the asset panel, in closed form,
with $M[i,j] = |{\rm coef}(x_{i,t-1}\!\to\!x_{j,t})|$, thresholded per window
...
--- 28 allocator--method--window cells, plus sparsity, cost and comparator
sweeps --- ran against 836 cached graph fits with a verified 100\% hit rate
and zero refits, at ${\approx}40$ seconds per backtest instead of
${\approx}15$ hours. The cache is also what \emph{guarantees} the ablation's
...
significantly-worse closed loop) did not survive the fix. All numbers in
this report come from the corrected, deterministic pipeline: two independent
runs now produce byte-identical windows, weights and NAVs --- verified
end-to-end by the replication gate of Section~\ref{sec:ablation-design}. A
result that does not survive a determinism fix was never real; treating
reproducibility as a prerequisite rather than an afterthought is part of the
method \cite{lopezdeprado2023causal}.
