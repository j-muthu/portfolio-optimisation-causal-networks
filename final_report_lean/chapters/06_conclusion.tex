...
\begin{enumerate}[itemsep=2pt]
  ...
  \item \textbf{Causal \emph{structure} survives data-snooping correction, but
        causal driver \emph{selection} does not.} Subjecting the \rsNtrials\
        configurations to the Deflated Sharpe ratio, White's Reality Check,
        Hansen's SPA and the Model Confidence Set, the absolute Sharpe levels
        survive correction for non-normality and multiplicity, and at one year the
        best causal-structure variant beats the baseline even family-wise
        ($p=\rsSpaRC$). The $90\%$ Model Confidence Set retains only \rsMcsMembers;
        the correlation baseline V0 \rsMcsVzeroIn\ in it --- but neither is the
        driver-based DYNOTEARS-V1, which is eliminated alongside V0. So the part of
        Q that asks whether causal \emph{driver selection} beats correlation is
        answered in the negative; what survives is asset-only and non-Gaussian
        structure, and only at the short window.
\end{enumerate}
...
\begin{itemize}[itemsep=2pt]
  ...
  \item \textbf{What survives is structure, not the driver-selection thesis, and
        only at one window.} The family-wise battery
        (Section~\ref{sec:believe-rigour}) leaves only the asset-only V0$'$ and
        VARLiNGAM-V1 in the $90\%$ Model Confidence Set; the project's headline
        DYNOTEARS driver-selection method is eliminated alongside the baseline,
        and the surviving edge is confined to the one-year window. The precise
        claim in Q --- that causal \emph{driver selection} beats correlation ---
        is therefore not established; the affirmative evidence is for causal
        \emph{structure} at the short horizon only.
\end{itemize}
