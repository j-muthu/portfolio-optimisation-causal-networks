...
of information: correlation is undirected (it records that two
...
lopezdeprado2023causal}. The second is a deficit of interface:
...
first asks whether a better input to hierarchical allocation exists;
the second, whether an allocator can be built that consumes what the
...
of information. Its skeleton, the undirected pattern of which asset
pairs are causally linked, can be handed to an unmodified hierarchical
allocator by symmetrising the adjacency matrix. Its edge
...
graph's skeleton versus its edge orientations?}}
\end{quote}

\noindent Every term is operationalised once, here, and held fixed for the
whole study:
\begin{itemize}[itemsep=1pt, topsep=3pt]
  \item Hierarchical portfolio allocation: HRP's two-stage scheme
  ...
  \item The correlation matrix: the graph-blind control \HRP{} is
  ...
  \item A discovered causal graph: the contemporaneous asset--asset
  ...
  \item Performance improvement: out-of-sample (walk-forward), net-of-cost
    ...
    Hansen's SPA (the studentised successor of White's Reality Check) and
    ...
    in the project, while the SPA and MCS each run over the
    family stated for their question in Chapter~\ref{chap:evaluation}.
  \item Skeleton versus orientations: a fixed-graph ablation.
...
\begin{enumerate}[label=Requirement \arabic*., itemsep=1pt, topsep=3pt]
  \item The causal graph must improve on the correlation matrix
    at all: the question's first part, and the headline motivation for
    the attribution part. The decomposition's internal contrasts remain
    informative about where any gain comes from even if it fails.
    Tested like-for-like: the
    same HRP machinery fed the symmetrised graph (\skelsamp{}) versus the sample
    correlation (\HRP{}), in Section~\ref{sec:r1}.
  \item An allocator must be able to exploit orientation. Symmetric
...
contrasts with a
shared anchor,
answered in order in Chapter~\ref{chap:evaluation}. The skeleton
contribution, $\skelsamp{}-\HRP{}$, measures the value of knowing which
pairs are
linked; the orientation contribution, $\orientany{}-\skelsamp{}$,
