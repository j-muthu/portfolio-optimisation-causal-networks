...
the object at the centre of the research question: a credible, reproducible
asset--asset causal
...
persistence between adjacent rebalances was not quantified, so the graphs'
stability over time is attested only by the determinism of the fits and
the stability of these aggregate diagnostics, a gap recorded in
...
Chapter~\ref{chap:evaluation} observes. \textbf{ridge-Granger} supplies
...
reuse one fit. The effect is decisive: the full ablation grid (every
...
not reproducible at the bit level, and two conclusions previously drawn
in an earlier phase of the project did not survive the fix. The same fix left
the VARLiNGAM graphs essentially unchanged: independent component analysis
(ICA) on VAR residuals is far
