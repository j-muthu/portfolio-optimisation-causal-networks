...
the research question answerable: a family of allocators fed the identical
per-window graph, split
...
discharges
Requirement 2: without it, the value of orientation cannot be measured at
...
halves, reversing an \emph{individual} edge can move mass between halves
...
the
asset-only Causal-HRP variant of the project's first phase, which allows the
...
experiment plan, plus two direction-free
covariance controls added during
...
harness reproduces
the committed first-phase bundle with $\Delta\text{Sharpe}=0$ and
