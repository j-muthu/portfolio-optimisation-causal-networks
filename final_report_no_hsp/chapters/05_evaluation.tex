...
the shared skeleton control (\skelsamp{} on DYNOTEARS graphs;
VARLiNGAM's variants are held to the same benchmark rather
...
estimation windows; they disagree at nine months, where the symmetrisation choice
...
skeleton gain, the orientation increment at long estimation windows, and the
...
estimation window; Section~\ref{sec:asset-graph}) supports, but does not prove, the
...
arm at long estimation windows, where both anchors agree.
...
Table~\ref{tab:regime-full} gives the headline variants alongside the
graph-blind control, so within-regime treatment-versus-control
differences can be
read directly): at
the
...
control
concentrates
in the bottom VIX quintile ($+0.29$/$+0.31$ Sharpe for
\skelsem{}/\orientsem{}) and is
slightly negative in the top
quintile ($-0.03$/$-0.05$), while in NBER
...
\paragraph{Family-wise comparisons (RC/SPA).} Two SPA families
...
second and labelled as such. The two families, \textbf{(i)}
the causal family vs the \HRP{} control and \textbf{(ii)} the
...
family only, marginal rejections confined to the longest estimation windows
...
costs nothing and improves robustness to the estimation-window choice,
though the same improvement is available from de-factoring the
covariance with no graph at all; and the edge at long estimation windows is suggestive
...
40 cells); the estimation-window dependence of that gain's composition (skeleton
...
controls as de-factoring rather than directional content; and a
narrowed-family edge over the stronger controls at the longest estimation windows,
marginal and reported as such.
